...
class CLOTHES(data.Dataset):
    def __init__(
        ...
        ## generate dataset
        self.hr_filenames = sorted(glob.glob(os.path.join(self.HR_folder, '*' + self.img_postfix)))
        self.lr_filenames = sorted(glob.glob(os.path.join(self.LR_folder, '*' + self.img_postfix)))
        self.nums_trainset = len(self.hr_filenames)

        LEN = len(self.hr_filenames)
        hr_dir = os.path.join(self.cache_dir, 'clothes_hr', 'ycbcr' if self.colors==1 else 'rgb')
        lr_dir = os.path.join(self.cache_dir, 'clothes_lr_x{}'.format(self.scale), 'ycbcr' if self.colors==1 else 'rgb')
        if not os.path.exists(hr_dir):
            os.makedirs(hr_dir)
        else:
            for i in range(LEN):
                hr_npy_name = self.hr_filenames[i].split('/')[-1].replace(self.img_postfix, '.npy')
        ...
        if not os.path.exists(lr_dir):
            ...
            for i in range(LEN):
                lr_npy_name = self.lr_filenames[i].split('/')[-1].replace(self.img_postfix, '.npy')
